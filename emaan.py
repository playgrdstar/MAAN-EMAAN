...
# Note: Results will be close, but there will be slight variation in results due to the non-deterministic nature of the Pytorch modules in this VAE framework
import os
import numpy as np
import pandas as pd
import json
...
mode = 'mul' # 'add' for EMAAN-Additive and 'mul' for EMAAN-Multiplicative
...
load_multidata(edgelist_df, imagefeature_df, wordfeature_df, description_df, expanded_genre_df, expanded_class_comp_df, threshold=0)


adj_orig = adj # only for val and test
...
adj_label = adj_label # for training only
...
if mode == 'add':
    from models import GATConvConcat as GATConv
elif mode == 'mul':
    from models import GATConvMul as GATConv
...
learning_rate = 0.02
n_epochs = 1000 # for training the main model, 5000 epochs per paper will take 6 hours, but running ~500 epochs will already give pretty good results
